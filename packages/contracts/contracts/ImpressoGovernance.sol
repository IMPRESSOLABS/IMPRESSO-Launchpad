    ...
    UUPSUpgradeable
{
    ...
    function finalizeProposal(uint256 _proposalId) external nonReentrant whenNotPaused {
        require(_proposalExists(_proposalId), "Proposal does not exist");
        Proposal storage proposal = proposals[_proposalId]; // <-- Move this up
        require(block.timestamp > proposal.endTime, "Voting period not ended");
        ProposalState state = getProposalState(_proposalId);
        require(state == ProposalState.Active || state == ProposalState.Succeeded || state == ProposalState.Defeated, "Proposal not finalizable");
